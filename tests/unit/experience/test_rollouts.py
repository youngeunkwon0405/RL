...
def cluster():
    ...
    cluster_name = f"test-rollout-cluster-{id(cluster)}"  # Unique name
...
@pytest.fixture(scope="function")
def multi_step_calculator_environment(cluster):
    env_actor = None
...
@pytest.fixture(scope="function")
def initial_multi_step_calculator_batch(tokenizer):
...
def multi_step_setup_vllm(
    ...
    try:
        ...
        vllm_config["tokenizer_name"] = tokenizer.name_or_path
        if "gpt2" in tokenizer.name_or_path.lower():
...
def test_run_multi_step_calculator_vllm(multi_step_setup_vllm):
    ...
    vllm_generation, tokenizer, task_to_env, initial_batch, cluster = (
...
@pytest.fixture(scope="function")
def sliding_puzzle_environment(cluster):
    env_actor = None
...
@pytest.fixture(scope="function")
def initial_sliding_puzzle_batch(tokenizer):
