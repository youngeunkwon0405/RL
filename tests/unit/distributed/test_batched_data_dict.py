...
from nemo_rl.distributed.batched_data_dict import BatchedDataDict, DynamicBatchingArgs, SequencePackingArgs
...
def test_sequence_packing_basic():
    ...
    for shard in sharded_batches:
        ...
        for chunk_indices, chunk_lengths in zip(shard.micro_batch_indices, shard.micro_batch_lengths):
        ...
        for s in sharded_batches:
            for mb in s.make_microbatch_iterator_for_packable_sequences():
                ...
                for i in range(mb['input_ids'].shape[0]):
                    ...
                    assert problem_id not in problem_ids_seen, f"Problem ID {problem_id} seen twice"
...
def test_sequence_packing_uniform_lengths():
    ...
    len_0 = len(list(sharded_batches[0].make_microbatch_iterator_for_packable_sequences()))
    len_1 = len(list(sharded_batches[1].make_microbatch_iterator_for_packable_sequences()))
...
def test_sequence_packing_invalid_algorithm():
    ...
    sequence_packing_args = SequencePackingArgs(
        ...
        input_key='input_ids',
        input_lengths_key='sequence_lengths',
        algorithm='invalid_algorithm'  # Invalid algorithm name
    )
    
    with pytest.raises(AssertionError, match="Only 'modified_ffd' algorithm is supported"):
...
def test_sequence_packing_with_dynamic_batching_conflict():
    ...
    with pytest.raises(AssertionError, match="dynamic_batching_args and sequence_packing_args cannot be passed together"):
        batch_data.shard_by_batch_size(
            shards=2,
            sequence_packing_args=sequence_packing_args,
...
def test_sequence_packing_microbatch_boundaries(pad_to_multiple_of):
    ...
    for gb_idx in range(num_global_batches):
        ...
        max_mb_count = 0
        legal_problem_ids = set(range(gb_idx * chunk_batch_size, (gb_idx + 1) * chunk_batch_size))
        for shard_idx in range(shards):
            ...
            for mb in shard_batch.make_microbatch_iterator_for_packable_sequences():
                ...
                for i in range(mb['input_ids'].shape[0]):
                    problem_id = mb['problem_ids'][i].item()
                    assert problem_id in legal_problem_ids, f"Problem ID {problem_id} not in legal problem IDs"
                    assert problem_id not in problem_ids_seen, f"Problem ID {problem_id} seen twice"
                    problem_ids_seen.add(problem_id)
                assert _get_padded_seqlen(mb['sequence_lengths']).sum().item() <= max_tokens_per_microbatch, f"Sequence length {_get_padded_seqlen(mb['sequence_lengths']).sum().item()} is greater than max tokens per microbatch {max_tokens_per_microbatch}"
        ...
        num_actual_tokens = sum(sequence_lengths[gb_idx * chunk_batch_size:(gb_idx + 1) * chunk_batch_size])
        packing_efficiency = num_actual_tokens / (mb_count_for_gb * max_tokens_per_microbatch)
        ...
        assert packing_efficiency >= pack_efficiency_standards[pad_to_multiple_of][0], f"We expect packing efficiency to be above {pack_efficiency_standards[pad_to_multiple_of][0]} for these nice random inputs with padding to multiples of {pad_to_multiple_of}. Got {packing_efficiency}"
        assert packing_efficiency <= pack_efficiency_standards[pad_to_multiple_of][1], f"We expect packing efficiency to be below {pack_efficiency_standards[pad_to_multiple_of][1]} for these nice random inputs with padding to multiples of {pad_to_multiple_of}. Got {packing_efficiency}"
    ...
    assert not torch.all(reconstructed['sequence_lengths'] == batch_data['sequence_lengths'])
    ...
    assert torch.all(reconstructed['sequence_lengths'] == batch_data['sequence_lengths'])