...
We recommend using the `uv` Python package installer for managing dependencies in NeMo RL.
...
1. The driver script creates several {py:class}`RayWorkerGroup <nemo_reinforcer.distributed.worker_groups.RayWorkerGroup>`.
