...
checkpointing.checkpoint_dir="results/grpo_8b_megatron_opt1234" \
...
logger.wandb.name='1017-grpo-8B-opt1234-megatron-${n_train_nodes}T${n_gen_nodes}G'" \
...
sbatch \
    ...
    --account=${account} \
    --job-name=${account}-async-megatron-opt1234.test \
    --partition=batch \
    --time=04:00:00 \
    --gres=gpu:8 \
    ray.sub
