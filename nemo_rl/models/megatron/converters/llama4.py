...
def split_qkv_llama(gathered_mcore_qkv_layer, cfg):
    ...
    num_query_groups = (
        ...
        if hasattr(cfg, "num_query_groups") and cfg.num_query_groups is not None
        else head_num
    )  # different num_query_groups for 70B / Llama 3

    head_size = (
        cfg.kv_channels
        if hasattr(cfg, "kv_channels") and cfg.kv_channels is not None
        else (hidden_size // head_num)
...
def split_fc1_gate_down_llama_common(
    gathered_mcore_fc1, cfg, gate_proj_name, up_proj_name
):
...
def split_fc1_gate_down_llama4_dense(gathered_mcore_fc1, cfg):
    ...
    return split_fc1_gate_down_llama_common(
        gathered_mcore_fc1, cfg, gate_proj_name, up_proj_name
    )
...
def split_shared_fc1_llama4(gathered_mcore_fc1, cfg):
    ...
    gate_proj_name = (
        "language_model.model.layers.{gl}.feed_forward.shared_expert.gate_proj.weight"
    )
    up_proj_name = (
        "language_model.model.layers.{gl}.feed_forward.shared_expert.up_proj.weight"
    )
    return split_fc1_gate_down_llama_common(
        gathered_mcore_fc1, cfg, gate_proj_name, up_proj_name
    )
...
mcore_te_to_hf_llama4 = {
    ...
    },  # Needs vocab pruning if nemo vocab > hf vocab
    "output_layer.weight": {
        "tp": 0,
