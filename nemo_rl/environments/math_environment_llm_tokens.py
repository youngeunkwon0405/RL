...
class HFVerifyWorker:
    ...
    def verify_response_format(self,response: str,num_tokens: float) -> bool:
        ...
        # Count <new_wordN> tokens
        tokens = re.findall(r"<new_word(?:[0-9]|[1-9][0-9])>", think_text)
        # score=float(min(len(tokens),50.0)/50.0) - num_tokens/8192.0 + 1
        score = float(min(len(tokens), 50.0) / 50.0) *(  1-num_tokens / 8192.0)
        return True, score

    def verify(
        self, pred_responses: List[str], ground_truths: List[Tuple]
    ) -> List[float]:
        ...
        results = []
        # ground_truth
        for response, tup in zip(pred_responses, ground_truths):
            ground_truth , num_tokens = tup
            ...
            try:
                ...
                format_bool, format_score = self.verify_response_format(response,float(num_tokens[0]))
                with _mute_output():
                    try:
                ...
                results.append(float(ret_score)) #results.append(format_score*float(ret_score))
            except Exception:
                results.append(0.0)
            # else:
            #     results.append(0.0)
    ...
    def step(
        ...
        for conversation in message_log_batch:
            assistant_responses = [
                interaction["content"]
                for interaction in conversation
                if interaction["role"] == "metadata"
            ]
            tokens_length.append(assistant_responses)
        ...
        ground_truths = [(gt,ctl) for ctl,gt in zip(tokens_length,ground_truths)]
        chunked_ground_truths = chunk_list_to_workers(ground_truths, self.num_workers)

        # chunked_tokens_length = chunk_list_to_workers(tokens_length, self.num_workers)
