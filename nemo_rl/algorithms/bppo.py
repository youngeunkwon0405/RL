...
class BPPOSaveState(TypedDict):
    step: int
    val_reward: float
    consumed_samples: int


def _default_bppo_save_state() -> BPPOSaveState:
    return {
        "step": 0,
...
class BPPOLoggerConfig(LoggerConfig):
...
def setup(
    ...
    """Main entry point for running BPPO algorithm.
    ...
    bppo_config = master_config["bppo"]
    logger_config = master_config["logger"]
    cluster_config = master_config["cluster"]

    assert generation_config is not None, (
        "A generation config in the PolicyConfig is required for BPPO"
    )
    ...
    bppo_save_state: Optional[BPPOSaveState] = checkpointer.load_training_info(
        last_checkpoint_path
    )
    if bppo_save_state is None:
        bppo_save_state = _default_bppo_save_state()
    ...
    if master_config["checkpointing"]["enabled"]:
        ...
        assert (
            ...
            % master_config["bppo"]["val_period"]
            == 0
        ), (
            f"Checkpointing save period {master_config['checkpointing']['save_period']} "
            f"must be a multiple of validation period {master_config['bppo']['val_period']}"
    ...
    dataloader = StatefulDataLoader(
        dataset,
        batch_size=int(
            bppo_config["num_prompts_per_step"]
            / bppo_config["bottom_percentile_to_keep"]
        ),
        shuffle=False,
    ...
    if bppo_config["val_period"] > 0 or bppo_config["val_at_start"]:
        assert val_dataset is not None, (
            "Validation dataset is required if validation is enabled"
        )
        val_dataloader = StatefulDataLoader(
            val_dataset,
            batch_size=bppo_config["val_batch_size"],
            shuffle=False,
            collate_fn=rl_collate_fn,
        )
...
def bppo_train(
    ...
    bppo_save_state: BPPOSaveState,
    master_config: MasterConfig,
) -> None:
    """Run BPPO training algorithm."""
    timer = Timer()
    NEED_REFIT = True
    ...
    step = bppo_save_state["step"]
    consumed_samples = bppo_save_state["consumed_samples"]
    val_period = master_config["bppo"]["val_period"]
    val_at_start = master_config["bppo"]["val_at_start"]
    ...
    # Run bppo training (single-turn)
    batch: BatchedDataDict[DatumSpec]
    for batch in dataloader:
        print(
            f"\n{'=' * 25} Step {step + 1}/{min(len(dataloader), master_config['bppo']['max_num_steps'])} {'=' * 25}"
        )
        ...
        with timer.time("total_step_time"):
            ...
            with timer.time("data_processing"):
                ...
                repeated_batch: BatchedDataDict[DatumSpec] = batch.repeat_interleave(
                    master_config["bppo"]["num_generations_per_prompt"]
                )
            ...
            with timer.time("generation"):
                repeated_batch, rollout_metrics = run_multi_turn_rollout(
                    ...
                    max_rollout_turns=master_config["bppo"]["max_rollout_turns"],
                    greedy=False,
                )
                policy_generation.finish_generation()
            ...
            with timer.time("reward_calculation"):
                ...
                baseline, std = calculate_baseline_and_std_per_prompt(
                    ...
                    leave_one_out_baseline=master_config["bppo"][
                ...
                if master_config["bppo"]["normalize_rewards"]:
            ...
            is_last_step = step + 1 == min(
                master_config["bppo"]["max_num_steps"], len(dataloader)
            )
            ...
            ## Checkpointing
            consumed_samples += master_config["bppo"]["num_prompts_per_step"]
            if master_config["checkpointing"]["enabled"] and (
                ...
                bppo_save_state["step"] = step + 1
                bppo_save_state["val_reward"] = val_metrics["accuracy"]
                bppo_save_state["consumed_samples"] = consumed_samples
                with timer.time("checkpointing"):
                    print(f"Saving checkpoint for step {step + 1}...")
                    checkpoint_path = checkpointer.init_tmp_checkpoint(
                        step + 1, bppo_save_state, master_config
                    )
        ...
        timer.reset()
        step += 1
        if step >= master_config["bppo"]["max_num_steps"]:
            break
...
def validate(
    ...
    with timer.time("total_validation_time"):
        ...
        max_batches = (
            master_config["bppo"]["max_val_samples"]
            // master_config["bppo"]["val_batch_size"]
        )
