...
class HfPolicy(PolicyInterface, GenerationInterface):
    ...
    def stream_weight_update(self, remote_workers, update_weight_method_name):
        worker_handles = ray.get(
            [
                worker.stream_weight_update.remote(
                    remote_workers, update_weight_method_name
                )
                for worker in self.worker_group.workers
            ]
        )

        assert all(worker_handles) == True
