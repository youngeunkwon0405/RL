...
def message_log_to_flat_messages(
    ...
    ```{doctest}
    >>> import torch
    >>> from nemo_reinforcer.data.llm_message_utils import message_log_to_flat_messages
...
def batched_message_log_to_flat_message(
    ...
    Args:
        ...
        make_divisible_by: forces the data to be divisible by this value
    ...
    ```{doctest}
    >>> import torch
    >>> from nemo_reinforcer.data.llm_message_utils import batched_message_log_to_flat_message
    >>> from nemo_reinforcer.distributed.batched_data_dict import BatchedDataDict
    ...
    if max_len % make_divisible_by != 0:
        max_len = ((max_len // make_divisible_by) + 1) * make_divisible_by
