...
from nemo_rl.data.hf_datasets.custom_dataset import CustomDataset
...
def custom_data_processor(
    ...
    print("initial user_message: ", user_message)
...
def setup_data(tokenizer: AutoTokenizer, data_config: DataConfig, env_configs):
    ...
    # Load custom using nemo rl datasets
    if data_config["dataset_name"] == "custom":
        print("Loading custom dataset for training and validation")
        data = CustomDataset(
    ...
    task_data_processors = defaultdict(
        lambda: (math_task_spec, custom_data_processor)
    )
    task_data_processors["math"] = (math_task_spec, custom_data_processor)
