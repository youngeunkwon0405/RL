...
def mathcl_data_processor(
    ...
    prompt = task_data_spec.prompt.format(problem)

    user_message = {"role": "user"}
    user_message["token_ids"] = tokenizer(prompt, return_tensors="pt")["input_ids"][0]
...
def setup_data(data_config: DataConfig, policy_config: PolicyConfig, env_configs):
    ...
    task_data_processors = defaultdict(lambda: (math_task_spec, mathcl_data_processor))
...
def main():
    ...
    if not args.config:
        args.config = os.path.join(os.path.dirname(__file__), "grpo_qwen_7b.yaml")
