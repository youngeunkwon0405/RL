...
from nemo_rl.environments.bfcl_environment import BFCLEnvironment
...
def setup_data(tokenizer: AutoTokenizer, data_config: DataConfig, env_configs):
    ...
    if "bfcl" in env_configs and env_configs["bfcl"]["enable"]:
        bfcl_env = BFCLEnvironment.options(
            runtime_env={
                "py_executable": BFCLEnvironment.DEFAULT_PY_EXECUTABLE,
                "env_vars": dict(os.environ),
            },
        ).remote(env_configs["bfcl"])
        task_to_env["bfcl"] = bfcl_env
