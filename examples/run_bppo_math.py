...
from nemo_rl.algorithms.bppo import MasterConfig, bppo_train, setup
...
def parse_args() -> tuple[argparse.Namespace, list[str]]:
    ...
    parser = argparse.ArgumentParser(description="Run BPPO training with configuration")
    parser.add_argument(
...
def main() -> None:
    ...
    if not args.config:
        args.config = os.path.join(
            os.path.dirname(__file__), "configs", "bppo_math_1B.yaml"
        )
    ...
    assert config["policy"]["generation"] is not None, (
        "A generation config is required for BPPO"
    )
