...
Usage:
    uv run python3 examples/llama4_refit.py --model_name /path/to/model
...
def parse_args():
    ...
    parser.add_argument(
        "--prompt", 
        type=str, 
        default="The following are multiple choice questions (with answers) about world religions.\n\nWhen was the first Buddhist temple constructed in Japan?\nA. 325 CE\nB. 119 CE\nC. 451 CE\nD. 596 CE\nAnswer:",
...
def analyze_logprob_differences(vllm_logprobs_data, megatron_generation_data, generation_data, tokenizer, prompt):
    ...
    if generated_length > 0:
        ...
        _detailed_token_comparison(
            vllm_gen_logprobs, megatron_gen_logprobs, 
            megatron_generation_data, input_length, total_length, tokenizer
        )
    else:
        print(f"No generated tokens to compare (input_length: {input_length}, total_length: {total_length})")


def _detailed_token_comparison(vllm_logprobs, megatron_logprobs, megatron_data, input_length, total_length, tokenizer):
    """
    ...
    Args:
        ...
        megatron_data: Megatron generation data
    ...
    if total_length > input_length:
        ...
        if "input_ids" in megatron_data:
            generated_tokens = megatron_data["input_ids"][0, input_length:total_length]
        else:
