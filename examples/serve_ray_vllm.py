...
async def send_request(client, messages, client_cfg, sample_idx, test_idx, extra_env_info):
...
def save_generations(generations, output_dir, batch_num):
    ...
    print(f"Saved {len(grouped_results)} prompts with {len(generations)} total responses to {filepath}")
    return filepath

def save_progress_state_and_generations(output_dir, completed_requests, batch_num, start_time, generations=None):
    ...
    if generations and len(generations) > 0:
        ...
        if filepath:
            print(f"Also saved generations grouped by prompt with progress state")
...
def save_progress_state(output_dir, completed_requests, batch_num, start_time):
    ...
    return save_progress_state_and_generations(output_dir, completed_requests, batch_num, start_time)
...
def load_progress_state(output_dir):
    ...
    if os.path.exists(state_file):
        ...
        print(f"Resuming from previous run. Already completed: {len(state['completed_requests'])} requests")
        return state
...
async def process_requests_concurrently(cfg):
    ...
    for i in range(0, len(all_requests), max_concurrent):
        ...
        print(f"Processing batch {i//max_concurrent + 1}/{(len(all_requests) + max_concurrent - 1)//max_concurrent}")
        ...
        for j, result in enumerate(batch_results):
            if result is not None and not isinstance(result, Exception):
                ...
                sample_idx, test_idx = batch_metadata[j]
                completed_requests.add(f"{sample_idx}_{test_idx}")
                processed_count += 1
        
        # Periodic progress save with generations
